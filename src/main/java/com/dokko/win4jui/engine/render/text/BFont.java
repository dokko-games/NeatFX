package com.dokko.win4jui.engine.render.text;

import com.dokko.win4jui.Win4JUI;
import com.dokko.win4jui.Logger4JUI;
import com.dokko.win4jui.engine.util.Files4JUI;
import com.dokko.win4jui.engine.util.Numbers4JUI;
...
public class BFont {
    ...
    public BFont(Font font, boolean antiAlias) {
        ...
        fontFile = new File(System.getProperty("user.home")+"/win4jui/fonts/"+getFontName()+"_"+str(font.getStyle())+"_"+font.getSize()+".fontmeta");
    ...
    private void saveMetaToFile() {
        ...
        if(Win4JUI.globalParameters.containsKey("STORE_FONTS") && Win4JUI.globalParameters.get("STORE_FONTS").equals(true)){
    ...
    private Texture loadMetaFromFile() {
        long size = Files4JUI.size(fontFile.getParentFile().toPath());
        if(size > 4000000000L){
            int dataI = (int)(size / 10000000L);
            float dataF = (float)dataI / 100;
            Logger4JUI.warn("More than 4GB of fonts found (%{0}GB). Clearing font data", dataF);
            Files4JUI.delete(fontFile.getParentFile());
            return null;
        }
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fontFile))) {
            ...
            if (dataLength != expectedLength) {
                Logger4JUI.error("Data length mismatch! Expected %{0}, but got %{1}", expectedLength, dataLength);
    ...
    private Texture createFontTexture(Font font, boolean antiAlias) {
        if(checkFontExists()){
            ...
            Logger4JUI.warn("Recalculating font data for font %{0}.", font.getFontName());
        }
        int margin = 2;
        // First, determine how many characters can fit in one line
        int charsPerLine = Numbers4JUI.nearestPerfectSquare(256, Numbers4JUI.NPS_MODE_ABOVE); // Assuming we have 256 characters
        ...
        Logger4JUI.warnDebug("Font %{0} %{1}: %{2}x%{3}",font.getFontName(), font.getSize(), imageWidth, imageHeight);
    ...
    private void reProcessFontMeta() {
        if(fontMeta.fontVersion != FontMeta.META_VERSION){
            Logger4JUI.error("Stored font metadata has a different version from the current: %{0} and %{1}", fontMeta.fontVersion, FontMeta.META_VERSION);
            Logger4JUI.warn("Will try to read font. Expect crashes");
        }
    ...
    /**
     * Ignore this, it is used by Win4JUI
     */
